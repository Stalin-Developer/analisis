...
<div class="card">
    <div class="card-body">
        ...
        <form action="<?= base_url('trabajos-titulacion/update/' . $trabajo['id']) ?>" method="POST" enctype="multipart/form-data">
            <?= csrf_field() ?>
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="linea_investigacion" name="linea_investigacion" 
                       value="<?= old('linea_investigacion', $trabajo['linea_investigacion']) ?>" required>
            </div>

            <div class="form-group">
                <label for="autores">Autor/es</label>
                <input type="text" class="form-control" id="autores" name="autores" 
                       value="<?= old('autores', $trabajo['autores']) ?>" required>
                ...
                <select class="form-control" id="career_id" name="career_id" required>
                    ...
                    <?php foreach ($careers as $career): ?>
                        <option value="<?= $career['id'] ?>" 
                                <?= old('career_id', $trabajo['career_id']) == $career['id'] ? 'selected' : '' ?>>
                            <?= $career['name'] ?>
                        </option>
                    <?php endforeach; ?>
                </select>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="mes_id" name="mes_id" required>
                            ...
                            <?php foreach ($meses as $mes): ?>
                                <option value="<?= $mes['id'] ?>" 
                                        <?= old('mes_id', $trabajo['mes_id']) == $mes['id'] ? 'selected' : '' ?>>
                                    <?= $mes['mes'] ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                        ...
                        <select class="form-control" id="academic_period_id" name="academic_period_id" required>
                            ...
                            <?php foreach ($academic_periods as $period): ?>
                                <option value="<?= $period['id'] ?>" 
                                        <?= old('academic_period_id', $trabajo['academic_period_id']) == $period['id'] ? 'selected' : '' ?>>
                                    <?= $period['name'] ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" id="resumen" name="resumen" style="resize: none;" rows="3"><?= old('resumen', $trabajo['resumen']) ?></textarea>
            </div>

            <div class="form-group">
                <label for="documento_path">Documento PDF (opcional)</label>
                <div class="custom-file">
                    <input type="file" class="custom-file-input" id="documento_path" name="documento_path" accept=".pdf">
                    <label class="custom-file-label" for="documento_path">Seleccionar nuevo archivo</label>
                </div>
            ...
            <!-- <div class="form-group">
                <label for="poster_path">Póster PDF (opcional)</label>
                <div class="custom-file">
                    <input type="file" class="custom-file-input" id="poster_path" name="poster_path" accept=".pdf">
                    <label class="custom-file-label" for="poster_path">Seleccionar nuevo archivo</label>
                </div>
                <small class="form-text text-muted">Deje en blanco para mantener el archivo actual</small>
            </div> -->
